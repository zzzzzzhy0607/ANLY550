...
if __name__ == "__main__":
    # This program does not take argument from sys
    # We try different cross-over points for different dimensions
    
    for dimension in (64, 126):
        
        # Generate random integers from [-10,10] as the matrix
        with open("input_file.txt", 'w') as wf:
            for i in range(2*dimension**2):
                wf.write(str(random.randint(-10, 10))+'\n')
        ...
        # Run time for conventional method
        t0 = time.time()
        conventional_product(A, B)
        t1 = time.time()
        time_conven = t1-t0
            
        # Write out the results into a file    
        with open("result.txt", "a") as file:
            print("dimension:", dimension)
            file.write(str(dimension)+"\n")            
            print("conventional:", time_conven)
            file.write(str(time_conven)+"\n")            
        
        # Now calculate the runtime for strassen's method
            for i in range(1, 300):
                if dimension >= i:
                    CrossOverPoint = i
                    # Run time for strassen's method
                    t2 = time.time()
                    C = strassen_even(A, B)
                    t3 = time.time()
                    time_stra = t3 - t2
                    
                    print("strassen's time:", time_stra, "for cross-over point", CrossOverPoint)
                    file.write(str(CrossOverPoint)+","+str(time_stra)+"\n")
            file.write("\n")
